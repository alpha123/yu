...
struct boxed_value {
  ...
  // NULL-terminated set of tags, contiguously layed out in memory
  value_t *tags;
...
// Returns true if `val` already has `tag` and false otherwise.
bool value_add_tag(value_t val, str_value tag);
bool value_has_tag(value_t val, str_value tag);

#define VALUE_TAGS_FOREACH(val, it) \
  for (value_t *_##it = (val)->tags; (it = *_##it); _##it++)
