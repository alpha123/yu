...
YU_ERR_RET yu_str_at(yu_str s, s64 idx, yu_str *char_out) {
    ...
    struct yu_str_dat *d = YU_STR_DAT(s);
    YU_THROWIF(idx >= 0 ? (u32)idx >= d->egc_count : (u32)labs(idx) > d->egc_count, YU_ERR_STRING_INDEX_OUT_OF_BOUNDS);
    u64 actual_idx = idx < 0 ? d->egc_count - (u32)labs(idx) : (u32)idx, start, len,
